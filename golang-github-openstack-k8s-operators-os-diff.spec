...
install -d -m0755 %{buildroot}%{_sysconfdir}/os-diff
install -Dp -m0644 os-diff.cfg %{buildroot}%{_sysconfdir}/os-diff/os-diff.cfg
install -Dp -m0644 config.yaml %{buildroot}%{_sysconfdir}/os-diff/config.yaml
install -Dp -m0644 ssh.config %{buildroot}%{_sysconfdir}/os-diff/ssh.config

%files
%license LICENSE
%doc README.md
%{_bindir}/*
%config(noreplace) %{_sysconfdir}/os-diff/*
